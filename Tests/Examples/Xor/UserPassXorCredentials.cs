...
public class UserPassXorCredentials : AuthCredentialsBase
{
    public string Username { get; }
    public string Password { get; }
    public byte PatternNum { get; private set; }

    public UserPassXorCredentials(string username, string password) : base(0x81)
    {
        Username = username;
        Password = password;
    }
    ...
    public override async Task<bool> Authenticate(NetworkStream stream, CancellationToken token)
    {
        ...
        */
        ...
        message.Add(0x01);
        PatternNum = (byte)Random.Shared.Next(256);
        message.Add(PatternNum);
    ...
    public override async Task<bool> Validate(NetworkStream stream, CancellationToken token)
    {
        ...
        try
        {
            ...
            var patternNumBuffer = new byte[1];
            await stream.ReadExactlyAsync(patternNumBuffer, 0, 1, token);
            PatternNum = patternNumBuffer[0];
    ...
    {
        return new XorEncryption(PatternNum);
    }
}